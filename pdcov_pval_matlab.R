...
# setup input files
inputX <- file.path(tmpDir, "x_pdcov.csv")
inputY <- file.path(tmpDir, "y_pdcov.csv")
inputZ <- file.path(tmpDir, "z_pdcov.csv")
inputR <- file.path(tmpDir, "pdcov_replicates.csv")
...
# call the R function
val <- pdcov.test(dataX, dataY, dataZ, R=replicates[1,1])

# return the pdcorr value, which is read directly as a system output by Matlab
output <- c(val$statistic, val$p.value)

# write the output to a file, for matlab to read
outFile = file.path(tmpDir, "pdcov_matlab_output.csv")
